// Obtiene Amplify desde la librería cargada a través del CDN
const Amplify = window.Amplify || (window.aws_amplify && window.aws_amplify.Amplify);
const amplifyClientConfig = window.amplifyConfig;

if (!Amplify) {
    throw new Error('AWS Amplify no está disponible. Verifica que aws-amplify.min.js se cargue antes de app.js');
}

if (!amplifyClientConfig) {
    throw new Error('La configuración de Amplify no está disponible. Verifica la carga de amplify-config.js');
}

// Configura Amplify con la exportación generada por la CLI
Amplify.configure(amplifyClientConfig);
...
async function checkUser() {
    try {
        ...
        const userLabel = currentAuthenticatedUser.attributes.phone_number
            || currentAuthenticatedUser.attributes.email
            || currentAuthenticatedUser.username;
        document.getElementById('app-status').innerText = `Autenticado como: ${userLabel}`;
        await fetchMessages();
    } catch (e) {
...
async function register() {
    ...
    if (!email || !phone || !password) {
        alert('Debes completar correo, teléfono y contraseña.');
        return;
    }
    ...
    try {
        await Amplify.Auth.signUp({
            username: phone,
            password: password,
            attributes: { phone_number: phone, email }
        });
